...
def bar_counts(df, field, orientation=None, ascending=None):
  df_ = df.value_counts(field).reset_index(name='count')
...
def pie_counts(df, field):
  return px.pie(df.value_counts(field).reset_index(name='count'), field, 'count')
