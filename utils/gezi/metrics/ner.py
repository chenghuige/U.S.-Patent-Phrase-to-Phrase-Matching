# https://github.com/lonePatient/BERT-NER-Pytorch/blob/master/metrics/ner_metrics.py
...
class SeqEntityScore(object):
  ...
  def __init__(self, markup='bios', id2label=None):
    self.markup = markup
  ...
  def result(self, return_detail=False):
    origin = len(self.origins)
    found = len(self.founds)
    right = len(self.rights)
    recall, precision, f1 = self.compute(origin, found, right)
    ret = {'acc': precision, 'recall': recall, 'f1': f1}
