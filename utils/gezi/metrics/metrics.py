...
try:
  ...
  logging.warning('not using njit for auc might due to numpy version > 1.20')
  def _auc(actual, pred_ranks):
    n_pos = np.sum(actual)
    n_neg = len(actual) - n_pos
    return (np.sum(pred_ranks[actual == 1]) - n_pos *
...
def group_auc(labels, preds, uids, weighted=True):
  ...
  # print('num instances', len(labels), 'num users', len(group_flag),
  ...
  for i in tqdm(range(num_users), total=num_users, ascii=True, desc='auc'):
    auc_score = 1. - inverse_ratio(group_truth[uids[i]], group_pred[uids[i]])
    ...
    impression = len(group_truth[uids[i]]) if weighted else 1
    # impression = 1
    if auc_score >= 0 and len(group_truth[uids[i]]) >= 2: 
      auc_score *= impression
  ...
  # ic(auc_scores, total_auc, total_impression)
