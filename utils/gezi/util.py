...
def trunct(l, maxlen, mark=0, last_tokens=0, use_sep=False, sep_token=None):
  if isinstance(l, np.ndarray):
    l = list(l)
    return np.asarray(trunct(l, maxlen, mark, last_tokens))
...
def parallel_run_list(func, inputs, num_workers=None, desc='run', verbose=1, leave=False):
  # timer = gezi.Timer()
  # num_workers = 1
...
def parallel_run_dict(func, inputs, num_workers=None, desc='run', verbose=1, leave=False):
  # timer = gezi.Timer()
  # num_workers = 1
  if num_workers is None:
    num_workers = len(inputs)
...
def parallel_run_loop(func, inputs, num_workers=None, desc='run', verbose=1, leave=False):
  # timer = gezi.Timer()
  # num_workers = 1
  if num_workers is None:
    num_workers = len(inputs)
...
def read_tsv(file, header=None, sep='\t', **kwargs):
  if not header:
...
def prepare_kaggle_dataset(kmodel_name,
  ...
  method = method if method is not None else FLAGS.kaggle_dataset_method
  ic(method)
...
def set_fold(df, folds, group_key=None, stratify_key=None, seed=1024):
  ...
  if stratify_key is None:
    ...
    if group_key is not None:
      ...
      fold_values = [rng.integers(folds) for _ in range(ngroups)]
      group2fold = dict(zip(group_values, fold_values))
      df['fold'] = df[group_key].map(group2fold)
    else:
      df['fold'] = [rng.integers(folds) for _ in range(len(df))]
  else:  
...
class Ensembler(object):
  def __init__(self, need_sort=False, ignores=['id', 'ids', 'label', 'labels'], includes=[], id_key='id'):
    self.x = None
