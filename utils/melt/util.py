...
def latest_checkpoint(model_dir, torch=False):
  if torch or FLAGS.torch:
    files = glob.glob(f'{model_dir}/*.tar') + glob.glob(f'{model_dir}/*.pt') + glob.glob(f'{model_dir}/*.bin')
...
def model_name_from_args(ignores=[], sep='.'):
  args = sys.argv[1:]
  ignores += ['mn', 'mns', 'mts', 'model_name',  'model_dir', 'cf', 'clear_first', 'gpus',
              'fold', 'folds', 'online', 'async_valid', 'async_eval', 'seed', 'num_workers',
              'mode', 'nvs', 'train_scratch', 'scratch', 'debug', 'test', 'restore_configs',
              'first_interval_epoch', 'fie', 'train_allnew', 'allnew', 'train_only', 'loop_only',
              'pretrain', 'pretrained', 'gpus', 'num_gpus', 'eval_batch_size', 'eval_bs', 
              'steps', 'num_train', 'num_valid', 'folds_metrics', 'folds_metrics_force', 'gradc', 'grad_acc',
              'kaggle_idx', 'kidx', 'kaggle_name', 'kd_method', 'kdmethod', 'kmethod', 
              'kaggle_dataset_mehod', 'kaggle_all', 'kall', 'tf_dataset', 'tfds', 'torch_only', 'to',
              'hug_caching', 'caching', 'hug_pb', 'hug_err', 'run_eagerly', 'pymp', 'wandb',
              'ensemble_metrics', 'enm', 'save_pred', 'save_csv', 'do_test', 'verbose', 'hack_infer',
              'out_hook_per_batch', 'rich_tqdm', 'r_tqdm', 'save_optimizer', 'save_scheduler',
              'restore_optimizer', 'restore_scheduler', 'workers', 'eval_leave', 'warmup_ratio'
              ]
  ignores = set(ignores)
  for arg in args:
    ...
    for ignore in ignores:
      if arg.split('=')[0] == f'--{ignore}':
        ...
        break
    if ok:
      FLAGS.mn += sep + arg.replace('flagfile', 'flag').replace('flags/', '').replace('--', '').replace('=', '-').replace(' ', '-').replace('/','_')
    ...
    for key, val_list in inputs.items():
      if key in max_lens:
        for i in range(len(val_list)):
          val_list[i] = pad_tensor(val_list[i], pad=max_lens[key], val=self.pad_vals.get(key, 0), dim=self.dim)
