...
def init(graph=None):
  ...
  if (not FLAGS.clear_first):
    ...
    if (not FLAGS.allow_loop) and model_exists and os.path.exists(f'{FLAGS.model_dir}/metrics.csv') \
      and  (FLAGS.mode == 'train' or FLAGS.mode is None) and (not FLAGS.steps < 0) \
        and (not FLAGS.folds_metrics) and (not FLAGS.kaggle_dataset_method):
  ...
  if FLAGS.restore_configs:
    gezi.restore_configs(
        ...
        ignore_names=[
            ...
            'valid_files', 'valid_inputs', 'test_files', 
            'workers',
        ])
  ...
  # ic(gezi.get('precision_policy_name'))
  ...
  if FLAGS.folds_metrics:
    ...
    # assert FLAGS.wandb and (not FLAGS.online)
    assert not FLAGS.online
    if not gezi.folds_done(FLAGS.model_dir, FLAGS.folds):
      FLAGS.wandb = False
